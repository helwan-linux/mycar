	...
	if os.path.exists(SETTINGS_FILE):
		try:
			with open(SETTINGS_FILE, 'r') as f:
				return {**default_settings, **json.load(f)} # دمج البيانات المحفوظة مع الافتراضية
...
def load_player_data():
	# يتم تحميل إعدادات اللعبة أولاً لأنها ضرورية للقيم الافتراضية
	...
	default_data = {
		...
		'car_type': 0,
		'dodged': 0,
		'player_name': 'Racer' # ⭐️ تم إضافة اسم اللاعب الافتراضي
	}
	if os.path.exists(DATA_FILE):
		try:
			with open(DATA_FILE, 'r') as f:
				loaded_data = json.load(f)
				# دمج البيانات المحفوظة مع الافتراضية للتأكد من وجود 'player_name'
...
PLAYER_NAME = PLAYER_DATA['player_name'] # ⭐️ تحميل اسم اللاعب
...
while running:
	...
	for event in pygame.event.get():
		if event.type == pygame.QUIT or (event.type == pygame.KEYDOWN and (event.key == pygame.K_ESCAPE or event.key == pygame.K_q)):
			...
			player_data_to_save = {
				...
				'car_type': car_type,
				'dodged': dodged,
				'player_name': PLAYER_NAME # ⭐️ حفظ اسم اللاعب
			}
	...
	# *********************************************************************
	if store_page:
		# كما اتفقنا: بما أن المتغيرات معرفة في النطاق الرئيسي، التعليق يحل المشكلة النحوية
	...
	if over_page:
		...
		# تحديث المسافة القياسية وحفظ بيانات اللاعب
		...
		if counter % COLLECTIBLE_SPAWN_INTERVAL == 0:
			...
			if spawn_type == 1:
				...
				for i in range(count):
					# تأكد أن فئة Coins في objects.py تم تحديثها لاستقبال coin_type
